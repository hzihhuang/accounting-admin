var a = Object.defineProperty;
var u = Object.getOwnPropertySymbols;
var d = Object.prototype.hasOwnProperty,
  m = Object.prototype.propertyIsEnumerable;
var i = (e, n, r) =>
    n in e
      ? a(e, n, { enumerable: !0, configurable: !0, writable: !0, value: r })
      : (e[n] = r),
  t = (e, n) => {
    for (var r in n || (n = {})) d.call(n, r) && i(e, r, n[r]);
    if (u) for (var r of u(n)) m.call(n, r) && i(e, r, n[r]);
    return e;
  };
import {
  d as s,
  ai as o,
  aS as p,
  aT as c,
  aU as y
} from "./index-DeFtvjuQ.js";
function g(e, n) {
  const r = /^IF-/;
  if (r.test(e)) {
    const f = e.split(r)[1],
      l = f.slice(0, f.indexOf(" ") == -1 ? f.length : f.indexOf(" ")),
      I = f.slice(f.indexOf(" ") + 1, f.length);
    return s({
      name: "FontIcon",
      render() {
        return o(p, t({ icon: l, iconType: I }, n));
      }
    });
  } else
    return typeof e == "function" ||
      typeof (e == null ? void 0 : e.render) == "function"
      ? n
        ? o(e, t({}, n))
        : e
      : typeof e == "object"
        ? s({
            name: "OfflineIcon",
            render() {
              return o(c, t({ icon: e }, n));
            }
          })
        : s({
            name: "Icon",
            render() {
              if (!e) return;
              const f = e.includes(":") ? y : c;
              return o(f, t({ icon: e }, n));
            }
          });
}
export { g as u };
