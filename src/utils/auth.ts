...
export function setToken(data: DataInfo) {
  ...
  if (data?.user && data.roles) {
    ...
    const avatar =
      storageLocal().getItem<DataInfo>(userKey)?.user?.avatarUrl ?? "";
    const username =
      storageLocal().getItem<DataInfo>(userKey)?.user?.username ?? "";
    const nickname =
      storageLocal().getItem<DataInfo>(userKey)?.user?.nickname ?? "";
